...
$(function () {
	...
	var tl_load = new TimelineMax({
		paused: true
	}).add(TweenMax.fromTo(loadingMask, durTime, { 'transform-origin': 'left center', transform: 'scale(0,1)' }, { 'transform-origin': 'left center', transform: 'scale(1,1)', ease: Expo.easeOut, delay: 0.3 }), 0).add(TweenMax.to(loading, .5, { 'transform-origin': 'right center', transform: 'scale(0,1)', ease: Expo.easeOut }), delayTime);
	...
	}).setClassToggle('#l-anchor', 'is-white').addTo(controller1);

	var scene2 = new ScrollMagic.Scene({
		triggerElement: '.p-topworks'
	}).setClassToggle('#l-anchor', 'is-white').addTo(controller2);

	//---------------------------------------------------
	// Resize Actions
	//---------------------------------------------------

	var $productBg = $('.p-topproduct__bg'),
	    $productLst = $('.p-topproduct__lst');

	$win.on('resize load', function () {

		var pLstMarginTop = 200,
